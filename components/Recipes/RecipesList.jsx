...
const RecipesList = () => {
  ...
  const { data: recipes, isLoading, error } = useGetTopRecipesQuery(filters);
  ...
  const handleSearch = () => {
    ...
    if (filterType === "name") newFilters.name = trimmed;
    else if (filterType === "category") newFilters.category = trimmed;
    else if (filterType === "ingredient") newFilters.ingredient = trimmed;
  ...
  if (error)
    return (
      <div>Error loading recipes: {error?.message || "Unknown error"}</div>
    );
  return (
    <div className="bg-gray-50 py-10">
      <div className="container mx-auto">
          ...
          >
            <div className="relative flex p-1 rounded-full bg-white border border-yellow-200 shadow-md md:p-2">
              <select
                ...
                className="p-3 rounded-full border border-yellow-200 shadow-md bg-white"
              >
